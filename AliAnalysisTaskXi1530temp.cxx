...
AliAnalysisTaskXi1530tempRunTable::AliAnalysisTaskXi1530tempRunTable(Int_t runnumber) {
    // Need to be Modified
...
AliAnalysisTaskXi1530temp::AliAnalysisTaskXi1530temp()
    : AliAnalysisTaskSE("AliAnalysisTaskXi1530temp"),
      fOption(),
      goodtrackindices(),
      goodcascadeindices(),
      fEMpool() {
...
//___________________________________________________________________
AliAnalysisTaskXi1530temp::AliAnalysisTaskXi1530temp(const char* name,
                                             const char* option)
...
AliAnalysisTaskXi1530temp::AliAnalysisTaskXi1530temp(const AliAnalysisTaskXi1530temp& ap)
    : fOption(ap.fOption),
      goodtrackindices(ap.goodtrackindices),
      goodcascadeindices(ap.goodcascadeindices),
      fEMpool(ap.fEMpool) {}
//___________________________________________________________________
AliAnalysisTaskXi1530temp& AliAnalysisTaskXi1530temp::operator=(
    const AliAnalysisTaskXi1530temp& ap) {
    // assignment operator

    this->~AliAnalysisTaskXi1530temp();
    new (this) AliAnalysisTaskXi1530temp(ap);
    return *this;
}
//___________________________________________________________________
AliAnalysisTaskXi1530temp::~AliAnalysisTaskXi1530temp() {
...
void AliAnalysisTaskXi1530temp::UserCreateOutputObjects() {
...
//________________________________________________________________________
void AliAnalysisTaskXi1530temp::UserExec(Option_t*) {
    // Pointer to a event----------------------------------------------------
    ...
    if (IsMC) {
        ...
        if (IsSelectedTrig)
            FillTHnSparse("htriggered_CINT7", {(double)kTrig,
                                               (double)fCent, ftrackmult});
        if (IsPS && IsGoodVertex)
            FillTHnSparse("htriggered_CINT7", {(double)kGoodVtx,
                                               (double)fCent, ftrackmult});
        if (IsPS && IsGoodVertex && IsVtxInZCut)
            FillTHnSparse("htriggered_CINT7", {(double)kVzCutted,
                                               (double)fCent, ftrackmult});
        if (IsINEL0Rec && IsMultSelcted)
            FillTHnSparse("htriggered_CINT7", {(double)kSelected,
                                               (double)fCent, ftrackmult});
    }
    // -----------------------------------------------------------------------
...
//________________________________________________________________________
Bool_t AliAnalysisTaskXi1530temp::GoodTracksSelection() {
...
Bool_t AliAnalysisTaskXi1530temp::GoodCascadeSelection() {
    ...
    if (fEvt->IsA() == AliESDEvent::Class()) {  // ESD case
        for (UInt_t it = 0; it < ncascade; it++) {
            ...
            if (fQA) {
                ...
                fHistos->FillTH1("hTPCPIDsignalLambdaProton",
                                    fTPCNSigProton);
                fHistos->FillTH1("hTPCPIDsignalLambdaPion",
                                    fTPCNSigLambdaPion);
            ...
            if (fQA) {
                fHistos->FillTH1("hDCADist_lambda_to_PV",
                                    fDCADist_Lambda_PV);
        ...
        for (UInt_t it = 0; it < ncascade; it++) {
            ...
            AliAODTrack* pTrackXi = (AliAODTrack*)(Xicandidate_aod->GetDaughter(0));
            ...
            */
            ...
            Double_t Xi_momsum = TMath::Sqrt(lMomXi[0]*lMomXi[0]+lMomXi[1]*lMomXi[1]+lMomXi[2]*lMomXi[2]);
            Double_t Xi_eta =
...
void AliAnalysisTaskXi1530temp::FillTracks() {
...
Double_t AliAnalysisTaskXi1530temp::GetMultiplicty(AliVEvent* fEvt) {
...
void AliAnalysisTaskXi1530temp::FillMCinput(AliMCEvent* fMCEvent, Int_t check) {
    ...
    for (Int_t it = 0; it < fMCEvent->GetNumberOfPrimaries(); it++) {
        ...
        if (check == 1)
            ...
            FillTHnSparse("hInvMass",
                          {(double)kDefaultOption, (double)kINEL10, (double)fCent,
                           mcInputTrack->Pt(), mcInputTrack->GetCalcMass()});
        else if (check == 3)
            FillTHnSparse(
                "hInvMass",
...
void AliAnalysisTaskXi1530temp::FillMCinputAOD(AliMCEvent* fMCEvent, Int_t check) {
    // Fill MC input Xi1530 histogram
    // check = 1: INELg0|10
    // check = 2: INEL10
    // check = 3: MB(V0AND)
...
void AliAnalysisTaskXi1530temp::FillMCinputdXi(AliMCEvent* fMCEvent, Int_t check) {
    // Fill MC input Xi1530 histogram
    // check = 1: INEL>0|10
    // check = 2: INEL10
    // check = 3: MB(V0AND)
    ...
    for (Int_t it = 0; it < fMCEvent->GetNumberOfPrimaries(); it++) {
        ...
        if (check == 1)
            FillTHnSparse("hInvMass_dXi",
                          {(double)kINELg010, (double)fCent,
                           mcInputTrack->Pt(), mcInputTrack->GetCalcMass()});
        else if (check == 2)
            FillTHnSparse(
                "hInvMass_dXi",
                {(double)kINEL10, (double)fCent,
                 mcInputTrack->Pt(), mcInputTrack->GetCalcMass()});
        else if (check == 3)
            FillTHnSparse(
                "hInvMass_dXi",
                {(double)kMCTruePS, (double)fCent,
                 mcInputTrack->Pt(), mcInputTrack->GetCalcMass()});
        else if (check == 4)
            FillTHnSparse(
                "hInvMass_dXi",
                {(double)kMCTrue, (double)fCent,
                 mcInputTrack->Pt(), mcInputTrack->GetCalcMass()});
    }
}
void AliAnalysisTaskXi1530temp::FillMCinputdXiAOD(AliMCEvent* fMCEvent,
                                               Int_t check) {
    // Fill MC input Xi1530 histogram
    // check = 1: INEL>0|10
    // check = 2: INEL10
...
THnSparse* AliAnalysisTaskXi1530temp::CreateTHnSparse(TString name,
                                                  TString title,
                                                  Int_t ndim,
...
Long64_t AliAnalysisTaskXi1530temp::FillTHnSparse(TString name,
...
Long64_t AliAnalysisTaskXi1530temp::FillTHnSparse(THnSparse* h,
                                              std::vector<Double_t> x,
                                              Double_t w) {
    // From AliPhysics/PWGUD/DIFFRACTIVE/Resonance/AliAnalysisTaskf0f2.cxx
    // Original author: Beomkyu Kim
...
TAxis AliAnalysisTaskXi1530temp::AxisFix(TString name,
                                     int nbin,
                                     Double_t xmin,
                                     Double_t xmax) {
    // From AliPhysics/PWGUD/DIFFRACTIVE/Resonance/AliAnalysisTaskf0f2.cxx
...
TAxis AliAnalysisTaskXi1530temp::AxisStr(TString name, std::vector<TString> bin) {
    // From AliPhysics/PWGUD/DIFFRACTIVE/Resonance/AliAnalysisTaskf0f2.cxx
    // Original author: Beomkyu Kim
...
TAxis AliAnalysisTaskXi1530temp::AxisVar(TString name, std::vector<Double_t> bin) {
    // From AliPhysics/PWGUD/DIFFRACTIVE/Resonance/AliAnalysisTaskf0f2.cxx
    // Original author: Beomkyu Kim
...
TAxis AliAnalysisTaskXi1530temp::AxisLog(TString name,
                                     int nbin,
                                     Double_t xmin,
                                     Double_t xmax,
                                     Double_t xmin0) {
...
Bool_t AliAnalysisTaskXi1530temp::IsMCEventTrueINEL0() {
    // From
...
Bool_t AliAnalysisTaskXi1530temp::IsTrueXi1530(AliESDcascade* Xi, AliVTrack* pion) {
...
Bool_t AliAnalysisTaskXi1530temp::IsTrueXi1530AOD(AliAODcascade* Xi,
                                               AliVTrack* pion) {
    // Check if associated Xi1530 is true Xi1530 in MC set
    if (!Xi)
        return kFALSE;
        ...
        if (MCLamD1esd->GetMother() ==
            ...
            if ((abs(MCLamD1esd->GetPdgCode()) == kProtonCode &&
                ...
                (abs(MCLamD1esd->GetPdgCode()) == kPionCode &&
                 abs(MCLamD2esd->GetPdgCode()) ==
                     kProtonCode)) {  // Lamda daugthers check #1
                MCLamesd = (AliAODMCParticle*)fMCArray
                               ->At(abs(MCLamD1esd->GetMother()));
                if (abs(MCLamesd->GetPdgCode()) ==
                    kLambdaCode) {  // Lambda check
                    if (MCLamesd->GetMother() ==
                        MCXiD2esd->GetMother()) {  // Lambda+pion(D2esd) mother check
                        MCXiesd = (AliAODMCParticle*)fMCArray
                                      ->At(abs(MCLamesd->GetMother()));
                        if (abs(MCXiesd->GetPdgCode()) ==
                            kXiCode) {  // Xi Check
                            MCXiStarD2esd = (AliAODMCParticle*)fMCArray
                                                ->At(abs(pion->GetLabel()));
                            if (MCXiesd->GetMother() ==
                                MCXiStarD2esd->GetMother()) {  // Xi+pion mother check
                                MCXiStaresd =
                                    (AliAODMCParticle*)fMCArray
                                        ->At(abs(MCXiesd->GetMother()));
                                if (abs(MCXiStaresd->GetPdgCode()) ==
                                    kXiStarCode) {  // Xi1530 check
                                    if (IsPrimaryMC) {
                                        if (MCXiStaresd->IsPrimary()) {
...
Bool_t AliAnalysisTaskXi1530temp::IsTrueXi(AliESDcascade* Xi) {
    // Check if associated Xi1530 is true Xi1530 in MC set
    if (!Xi)
        return kFALSE;
...
Bool_t AliAnalysisTaskXi1530temp::IsTrueXiAOD(AliAODcascade* Xi) {
    // Check if associated Xi1530 is true Xi1530 in MC set
    if (!Xi)
        return kFALSE;
    Bool_t TrueXi = kFALSE;
...
void AliAnalysisTaskXi1530temp::FillTrackToEventPool() {
