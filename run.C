...
#if !defined (__CINT__) || defined (__CLING__)
// ROOT 6 MODE
vector<Int_t> LHC16k_test = {257605}; // for test
         ...
         )
{
    ...
    //----GRID  MODE-------------------------------------------------
        ...
        if (foption.Contains("LHC16k")){
            ...
            for(auto i=0u;i<LHC16k.size();i++)
                plugin->AddRunNumber(LHC16kRuns.at(i));
        }
        if (foption.Contains("LHC16k_test")){
            if(ismc){
                plugin->SetGridDataDir("/alice/sim/2018/LHC18c6b4"); //resonance injected
                plugin->SetDataPattern("*AliESDs.root");
            }
            ...
            for(auto i=0u;i<LHC16k_test.size();i++)
                plugin->AddRunNumber(LHC16k_test.at(i));
        }
