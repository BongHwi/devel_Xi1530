...
TH1D * BSTHnSparseHelper::GetTH1( TString name, Int_t xDim, Int2D bin, Option_t*opt){
  ...
  for( UInt_t i=0;i<bin.size();i++ ){
    ...
    auto ax = GetAxis(i);
    //cout << "name: " << name << ", b: " << b[0] << ", " << b[1] << ", maxnbin: " << maxnbin << endl;
...
void BSTHnSparseHelper::LoadBinFromHist( int iaxis){
  auto ax = GetAxis(iaxis);
  fCBinsB[iaxis] = {0};
  //std::cout << "axis: " << iaxis << ", nbins: " << ax->GetNbins()+1 << std::endl;
  for( int ib=1;ib<=ax->GetNbins()+1;ib++ ){
    fCBins[iaxis].push_back( ax->GetBinLowEdge(ib) );
    fCBinsB[iaxis].push_back( ib );
    //std::cout << "fCBinsB[" << iaxis << "].at(" << ib << "): " << fCBinsB[iaxis].at(ib) << std::endl;
  } 
  //std::cout << "fCBinsB[" << iaxis << "].size(): " << fCBinsB[iaxis].size() << std::endl;
}
//__________________________________________________________
...
public:
  ...
  TPad * AddPadInGrid(TString name, Option_t *opt, int nx, int ny, double wx=-1, double wy=-1){
    if(name.EndsWith("-")) name += Form("%02d%02d",nx,ny);
    return AddPad( name, Form("%sgrid;",opt),-1, -1, wx, wy, nx, ny );
  }
  TPad * AddPadInGridNext(TString name, Option_t *opt, double wx=-1, double wy=-1){
    fGridCursorX++;
    if( fGridCursorX >= fGridXn ){
      fGridCursorX=0;
      fGridCursorY++;
    }
    return AddPadInGrid( name, opt, fGridCursorX,fGridCursorY, wx, wy );
  }
  ...
  TH2F *SetRange( TH1 *h ){
    return SetRange( h->GetXaxis()->GetXmin(), h->GetXaxis()->GetXmax(),h->GetMinimum(),h->GetMaximum() );
  }
  TH2F *SetMain( TH1 *h, Option_t* opt="", TString title="" ){
  ...
  TLegend* SetLegend(double x1, double y1, double x2, double y2, TString header="", Option_t*opt="brNDC"){
    ...
    l->SetHeader( header);
    l->SetX1(x1);l->SetX2(x2);l->SetY1(y1);l->SetY2(y2);
    return l;
  }
  TLegend* AddLegendEntry( TH1 *h, TString title="", Option_t* opt="p"){
    auto l = fLegends[fPadCursor];
  ...
  TH1* DrawHist( TH1 *h, Option_t* opt="", Option_t *gopt="",int color=0, int marker=20, TString legend="",Option_t* legendopt="p" ){
    auto hb = GetBaseHist();
    ...
    if(gopts.Contains("legend")) AddLegendEntry(h,legend,legendopt);
  ...
  TPad * AddPad( TString name, Option_t * opt, double x, double y, double wx, double wy, int nx=-1, int ny=-1 ){
